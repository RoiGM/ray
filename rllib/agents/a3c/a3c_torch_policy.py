...
def actor_critic_loss(policy, model, dist_class, train_batch):
    ...
    policy.value_err = nn.functional.mse_loss(
        out[SampleBatch.VF_PREDS].reshape(-1), train_batch[Postprocessing.VALUE_TARGETS], reduction='sum')
    overall_err = sum([
        policy.pi_err,
...
class ValueNetworkMixin:
    def _value(self, obs):
        _, _, extra_outs = self.model({SampleBatch.OBS: torch.Tensor([obs]).to(self.device)}, [], [1])
